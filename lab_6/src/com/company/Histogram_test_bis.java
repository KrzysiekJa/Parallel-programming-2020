...
import com.company.Classes.ObrazThreads;
import com.company.Classes.RunnableWatek;

import java.util.Scanner;


class Histogram_test_bis {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < num_threads; i++) {
            ...
            first = (int) (Math.ceil((float)obraz_1.getSize_n() / num_threads) * i);
            last = (int) (Math.ceil((float)obraz_1.getSize_n() / num_threads) * (i + 1));
            (NewThr[i] = new ObrazThreads(first, last, i, obraz_1)).start();
        }
